...
namespace ClinicsReservation.Controllers
{
    ...
    public class ClinicDoctorController : ControllerBase
    {
       ...
       /* [HttpGet]
        ...
        {
            var ClinicCheck = await _context.Clinics.FirstOrDefaultAsync(c => c.Id == clinicId);
            if (ClinicCheck == null)
            {
        ...
        {
            var DrCheck = await _context.Doctors.FirstOrDefaultAsync(d => d.Id == doctorId);
            if (DrCheck == null)
            {
            ...
            if (clinicDoctor.Count()<= 0)
            {
                return NotFound($"No Clinics found for this Doctor {DrCheck.DoctorName} ");
            }
